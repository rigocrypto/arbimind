...
describe('initSchema singleton', () => {
  ...
  it('allows retry after permanent failure (non-transient error)', async () => {
    const fatalErr = Object.assign(new Error('permission denied'), { code: '42501' });
    mockQuery.mockRejectedValueOnce(fatalErr);

    // First call fails permanently (non-transient code, no retry)
    await initSchema();
    expect(mockQuery).toHaveBeenCalledTimes(1);

    // Promise should be cleared so next call can retry
